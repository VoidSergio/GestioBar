...
**Stato:** Fase 0 chiusa, app pubblicata su Netlify. Fatti T-06 (accesso), T-07 (cache offline), T-08 (clienti), T-09 (coda), T-10 (griglia), T-11 (apertura conto), T-12 (righe), T-13 (chiusura conto), T-14 (scheda cliente), T-15 (crediti), T-16 (listino), T-17 (PWA), più **T-20 e T-22** (chiusura di turno) anticipati dalla Fase 2 — il perché è scritto in `05-ROADMAP.md`. 63 prodotti a catalogo, 402 test verdi. Il giro completo funziona. **T-18 è cominciato**: le prime tre correzioni dal banco sono in `09-DIARIO.md` alla data del 12 agosto.
...
**Una pagina che deve funzionare senza server non può passare dal server.** `/sw.js` e `/offline` stanno fuori dal `matcher` del proxy, e la regola vive in `lib/dominio/rotte.ts` con i test — non dentro un'espressione regolare in fondo a `proxy.ts`, dove nessuno la guarda. **E `fetch` non ha una scadenza**: ogni attesa di rete davanti a un utente ne vuole una a mano, altrimenti il caso peggiore non è l'errore ma l'attesa infinita (`09-DIARIO.md`, 13 agosto).

**Prima di aggiungere un divieto, cerca se c'è già.** Le chiavi esterne sono divieti, e stanno nel punto giusto: `creato_da` impedisce da sola di cancellare chi ha battuto qualcosa. Un trigger in più su `profili` ha bloccato per un giorno anche la cancellazione di un account creato per sbaglio, perché scattava sulla cascata da `auth.users`.
