...
**Stato:** Fase 0 chiusa, app pubblicata su Netlify. Fatti T-06 (accesso), T-07 (cache offline), T-08 (clienti), T-09 (coda), T-10 (griglia), T-11 (apertura conto), T-12 (righe), T-13 (chiusura conto), T-14 (scheda cliente), T-15 (crediti), T-16 (listino), T-17 (PWA), più **T-20 e T-22** (chiusura di turno) anticipati dalla Fase 2 — il perché è scritto in `05-ROADMAP.md`. 63 prodotti a catalogo, 347 test verdi. Il giro completo funziona. **T-18 è cominciato**: le prime tre correzioni dal banco sono in `09-DIARIO.md` alla data del 12 agosto.
